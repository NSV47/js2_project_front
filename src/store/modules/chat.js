...
export default {
  ...
  getters: {
    ...
    chatMessages: state => id => {
      // функция id, которая в себя принимает state
      return state.messages.filter(messages => messages.id == id);
    } // и далее возвращает(фильтр на вход получает messages и возвращает message.id == id)
  },
  mutations: {
